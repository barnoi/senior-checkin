...
export default function LandingPage() {
  ...
  const testimonials = [
    { name: "מיכל כהן", role: "בת לניצול שואה", text: "זה שינה לנו את הבוקר. במקום להתקשר בלחץ, אני מקבלת הודעה כשהיא שותה את הקפה.", img: "https://images.unsplash.com/photo-1508214751196-bcfd4ca60f91?w=80&h=80&fit=crop&crop=face" },
    { name: "דני לוי", role: "בן להורה יחיד", text: "סוף סוף אבא מרגיש שאני לא מעיק עליו, ואני רגוע שהוא התעורר והכל בסדר.", img: "https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=80&h=80&fit=crop&crop=face" },
    { name: "רחל אברהם", role: "סבתא ל-12", text: "הכפתור הירוק כל כך פשוט. אני לוחצת וכל הילדים שלי יודעים מיד שאני בסדר.", img: "https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=80&h=80&fit=crop&crop=face" },
    { name: "יוסי מזרחי", role: "מנהל הייטק", text: "השירות הזה נותן לי שקט נפשי באמצע יום עבודה עמוס. פשוט ויעיל.", img: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=80&h=80&fit=crop&crop=face" },
    { name: "אורית גל", role: "בת להורים בקיבוץ", text: "זה פשוט נותן לנו להתחיל את היום בידיעה שהכל בסדר, בלי להפריע לשגרה שלהם.", img: "https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=80&h=80&fit=crop&crop=face" },
    { name: "שמואל כץ", role: "גמלאי", text: "אני מרגש בטוח יותר בידיעה שהמשפחה שלי מחוברת אליי בלחיצת כפתור.", img: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?w=80&h=80&fit=crop&crop=face" }
  ];
    ...
    <div className="min-h-screen bg-white text-right text-slate-900" dir="rtl" suppressHydrationWarning>
      ...
      <nav className="p-4 md:p-6 max-w-6xl mx-auto flex justify-between items-center border-b border-slate-50 sticky top-0 bg-white/80 backdrop-blur-md z-50">
        <div className="flex flex-col items-center">
          ...
          <p className="text-[8px] md:text-[10px] font-bold text-blue-600 uppercase tracking-[0.2em] mr-[-0.2em] mt-1">גשר דיגיטלי למשפחה</p>
        </div>
        <div className="flex gap-2 md:gap-4 items-center">
          ...
          <button onClick={handleExistingUserLogin} className="bg-slate-100 px-3 py-2 rounded-lg text-slate-700 font-bold hover:bg-blue-600 hover:text-white transition text-[11px] md:text-sm whitespace-nowrap">
      ...
      <header className="py-8 md:py-16 px-6 max-w-6xl mx-auto flex flex-col md:grid md:grid-cols-2 gap-12 items-center">
        ...
        <div className="order-2 md:order-1 text-center md:text-right">
          <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-4">
            <span className="bg-blue-50 text-blue-800 px-3 py-1 rounded-lg text-[10px] font-bold italic">👩‍⚕️ פותח על ידי מרפאה בעיסוק</span>
            <span className="bg-green-50 text-green-800 px-3 py-1 rounded-lg text-[10px] font-bold">🇮🇱 מיוצר ומאוחסן בישראל</span>
            <span className="bg-slate-100 text-slate-700 px-3 py-1 rounded-lg text-[10px] font-bold">🔒 אבטחה מלאה</span>
          </div>
          ...
          <div className="bg-blue-50/50 border-r-4 border-blue-600 p-6 mb-8 text-right shadow-sm rounded-l-2xl">
            <p className="text-slate-800 font-bold mb-4 text-sm md:text-base leading-relaxed">נעים מאוד, אני מרפאה בעיסוק, עובדת עם הדור השלישי ובת להורים מתבגרים. במשך שנים ראיתי את המורכבות הזו מהצד המקצועי...</p>
            <p className="text-slate-700 text-sm md:text-base leading-relaxed mb-4 italic">"הסיפור האישי שלי התחיל בכל בוקר מחדש, כשהלב שלי היה מחסיר פעימה: האם להתקשר עכשיו? אולי הם עוד ישנים? אולי קרה משהו והם לא יכולים לענות?"</p>
            <p className="text-slate-800 font-medium text-sm md:text-base leading-relaxed mb-6">מצאתי את עצמי במלכודת – מצד אחד דאגה עמוקה, ומצד שני חוסר נעימות להפוך כל שיחה ל"בדיקת נוכחות" מעיקה. הבנתי שחסר לנו <span className="text-blue-700 font-black">גשר דיגיטלי חם ומכבד.</span></p>
          </div>
          <button onClick={() => document.getElementById('offer')?.scrollIntoView({ behavior: 'smooth' })} className="w-full md:w-auto bg-blue-600 text-white px-10 py-5 rounded-2xl font-black text-xl hover:bg-blue-700 transition shadow-xl hover:scale-105 active:scale-95">
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="p-8 rounded-3xl bg-slate-50 border border-slate-100 hover:shadow-md transition group">
              ...
              <p className="text-slate-600 leading-relaxed font-medium">ההורה יכול להתקשר אלייך (המכותב הראשי) או לכל אחד מהמלווים בלחיצה אחת מהירה מתוך המערכת, בלי להסתבך עם חיפוש אנשי קשר ברגע של צורך.</p>
            </div>
            <div className="p-8 rounded-3xl bg-slate-50 border border-slate-100 hover:shadow-md transition group">
              <div className="text-4xl mb-4 group-hover:scale-110 transition">💬</div>
              <h3 className="text-xl font-black mb-3 text-blue-600">עדכון אוטומטי למשפחה</h3>
              <p className="text-slate-600 leading-relaxed font-medium">ברגע שההורה לוחץ על הכפתור, נשלחת הודעת וואטסאפ מרגיעה לכל רשימת המלווים שהגדרתם מראש. כולם מעודכנים בבת אחת.</p>
            </div>
            <div className="p-8 rounded-3xl bg-blue-600 text-white shadow-xl hover:shadow-2xl transition group transform md:-translate-y-2">
              ...
              <p className="text-blue-50 leading-relaxed font-medium"><span className="font-black underline">זה הפלא האמיתי:</span> אם ההורה לא לחץ על הפעמון שעתיים אחרי המועד שקבעתם – המערכת שולחת לכם התראה אוטומטית: <span className="block mt-2 font-black italic">"כדאי שתבדקו, אמא לא עדכנה היום."</span></p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="py-10 bg-blue-600 px-6 text-center">
        <div className="max-w-2xl mx-auto">
          <p className="text-white font-black text-xl md:text-2xl mb-2">מוכנים לשקט נפשי אמיתי?</p>
          <p className="text-blue-100 text-sm mb-6 font-medium">הצטרפו למשפחות שכבר נרגעו — שנה שלמה ב-199 ₪</p>
          <button onClick={() => document.getElementById('offer')?.scrollIntoView({ behavior: 'smooth' })} className="bg-white text-blue-700 px-8 py-4 rounded-2xl font-black text-lg hover:bg-blue-50 transition shadow-xl hover:scale-105 active:scale-95">
            אני רוצה להצטרף ←
          </button>
        </div>
      </section>
            ...
            {testimonials.map((t, i) => (
              <div key={i} className="bg-slate-50 p-6 rounded-3xl border border-slate-100 shadow-sm italic text-right">
                <div className="flex justify-end mb-3">
                  <img src={t.img} alt={t.name} className="w-10 h-10 rounded-full object-cover border-2 border-blue-100" />
                </div>
          ...
          <div className="mt-6">
            ...
            <div className="space-y-4 mb-8">
              ...
              <div className="bg-blue-500/10 border border-blue-500/20 rounded-2xl p-4 max-w-sm mx-auto">
                <p className="text-sm text-slate-300 font-medium leading-relaxed">במחיר של <span className="text-blue-400 font-bold">כוס קפה אחת בחודש (17 ₪)</span>, אתם מבטיחים שההורה תמיד מוגן ומחובר אליכם.</p>
              </div>
            </div>
              ...
              {!paid ? (
                <div className="flex flex-col gap-6">
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        if (val === 'FREE-FAMILY20') {
                          ...
                          if (!email || !email.includes('@')) { alert("נא להזין כתובת מייל תקינה"); setCoupon(''); return; }
                          const cleanEmail = email.trim().toLowerCase();
                          await supabase.from('customers').upsert({ email: cleanEmail, payment_status: 'coupon', plan_type: 'lifetime' });
                  ...
                  <div className="bg-white p-4 rounded-2xl shadow-inner min-h-40 flex items-center justify-center overflow-hidden">
                    <PayPalButton amount="199.00" onSuccess={() => { setPaid(true); setTimeout(() => { router.push("/admin"); }, 2000); }} />
                  </div>
                  ...
                  <div className="text-center">
                    <button
                      ...
                      className="flex items-center justify-center gap-2 w-full bg-cyan-500 text-white py-4 rounded-2xl font-black text-base hover:bg-cyan-600 transition shadow-lg"
                    >
                      <span>💳</span>
                      <span>{showQR ? 'סגור QR ביט' : 'תשלום בביט — סרקו QR'}</span>
                    </button>
                    {showQR && (
                      <div className="mt-3 bg-white p-4 rounded-2xl shadow-inner">
                        <p className="text-slate-600 text-xs mb-3 font-bold">סרקו עם אפליקציית ביט לתשלום של 199 ₪</p>
                        <img
                          src="https://lytkztprahzwvusyhmke.supabase.co/storage/v1/object/public/contact-images/qrc%20bit.webp"
                          alt="QR ביט"
                          className="w-48 h-48 mx-auto rounded-xl"
                        />
                        <p className="text-slate-400 text-[10px] mt-2">לאחר התשלום שלחו אישור לוואטסאפ</p>
                      </div>
                    )}
                  </div>
      ...
      <section className="py-16 px-6 bg-white">
        ...
        <div className="max-w-2xl mx-auto space-y-4 text-right" dir="rtl">
          {[
            { q: "איך מוסיפים את SeniorSafe כ\"אייקון\" על מסך הבית?", a: "פתחו את האתר בטלפון. באייפון: לחצו על כפתור ה-\"שתף\" (מרובע עם חץ למעלה) ובחרו \"הוסף למסך הבית\". באנדרואיד: לחצו על שלוש הנקודות בדפדפן ובחרו \"התקן אפליקציה\" או \"הוסף למסך הבית\"." },
            { q: "איך אני מוצא/ת קישור (URL) לתמונה?", a: "העלו את התמונה הרצויה לוואטסאפ במחשב (WhatsApp Web), קליק ימני על התמונה ובחרו \"העתק כתובת תמונה\". הדביקו את הקישור שקיבלתם בהגדרות המלווה במערכת." },
            { q: "מהו פורמט מספר הטלפון המדויק?", a: "יש להזין פורמט בינלאומי ללא סימנים כלל. לדוגמה: 972541234567 (שימו לב: ללא ה-0 בהתחלה וללא סימן ה-+)." },
            { q: "מה קורה אם ההורה שוכח ללחוץ?", a: "המערכת תמתין עד לשעה המדויקת שהגדרתם. במידה ולא התקבל עדכון מההורה עד לאותה שעה, תשלח הודעת וואטסאפ אוטומטית לכל המלווים שרשמתם במערכת." },
            { q: "מה קורה בתום השנה? האם תהיה לי גישה לנתונים?", a: "נשלח לכם תזכורת ידידותית חודש לפני תום המנוי כדי שתוכלו לחדש אותו בקלות. לא ביצענו חיוב אוטומטי, כך שאתם תמיד בשליטה מלאה." },
            { q: "האם המחיר של 199 ₪ יישאר לי גם בשנה הבאה?", a: "כן! כחלק מנבחרת המייסדים (Founders), אנחנו מתחייבים שהמחיר המיוחד שלכם יישמר גם בחידושים הבאים." },
            { q: "199 ₪ לשנה? זה הכל?", a: "בדיוק. פחות מ-17 ₪ לחודש – שזה פחות ממחיר של כוס קפה אחת. אנחנו מאמינים שביטחון ושקט נפשי למשפחה צריכים להיות נגישים לכל אחד." }
          ].map((faq, index) => (
